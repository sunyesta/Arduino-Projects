...
void callback(char* topic, byte* payload, unsigned int length) {
    ...
    if ((char)payload[0] == '1') {
        digitalWrite(BUILTIN_LED,
                     LOW);  // Turn the LED on (Note that LOW is the voltage
                            // level but actually the LED is on; this is because
...
void reconnect() {
    ...
    while (!client.connected()) {
        ...
        // Attempt to connect
        if (client.connect(clientId.c_str(), "student039", "ApwcC3kY")) {
...
void loop() {
    ...
    if (now - lastMsg > 2000) {
        ...
        Serial.println(msg);
        client.publish("infob3it/039/plant/test", msg);
    }
}
